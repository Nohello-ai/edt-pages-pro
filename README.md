...
- **Ivory Instrument 视觉**：象牙浅色 + **毛玻璃卡片**（管理台）
- **登录页**：参考经典后台（全屏背景 + 居中白卡片），**仅密码、无用户名**
- **信息架构**：登录后默认「节点订阅」；左概览/优选；右「我的」；侧栏底 **高级设置开关**
- **静态资源绝对路径**：HTML 由 Worker 同源返回时，CSS/JS 仍从 GitHub Pages 加载
